...
public class ServicioSeguridad {
		...
		if (tab_usuario.getTotalFilas() > 0) {
			...
			if (tab_usuario.getValor("BLOQUEADO_USUA") == null || tab_usuario.getValor("BLOQUEADO_USUA").equals("false") || tab_usuario.getValor("BLOQUEADO_USUA").equals("0")) {
				...
				if (isUsuarioLogeado(tab_usuario.getValor("IDE_USUA")) == false) {
							...
							if (encriptar.getEncriptar(clave).equals(
								...
								try {
									if(isPerfilActivo(tab_usuario.getValor("IDE_PERF"))){
										...
										StringBuilder sbIde_perf=new StringBuilder();
										TablaGenerica tab_usuario_perfil = utilitario.consultar("select ide_uspe,ide_perf,ide_usua from sis_usuario_perfil where ide_usua = "+ tab_usuario.getValor("IDE_USUA"));
										sbIde_perf.append(tab_usuario.getValor("IDE_PERF"));
										
										for(int i=0;i<tab_usuario_perfil.getTotalFilas();i++)
										{
											if(sbIde_perf.toString().isEmpty()==false){
												sbIde_perf.append(",");
						                    }
											sbIde_perf.append(tab_usuario_perfil.getValor(i,"ide_perf"));	
										}

										utilitario.crearVariable("IDE_PERF", sbIde_perf.toString()); //utilitario.crearVariable("IDE_PERF", tab_usuario.getValor("IDE_PERF"));		//awbecerra tab_usuario.getValor("IDE_PERF") -> in (1,2,3)	//para multiples perfiles						
										System.out.println("ide_perf: "+sbIde_perf.toString());
					...
					str_mensaje = "El usuario ingresado tiene sessión iniciada en otra máquina, contactese con el administrador del sistema o espere 15 min y vuelva a intentarlo...";
				}
			} else {
	...
	public int getNumeroIntentosAcceso() {
		...
		if (tab_intentos.isEmpty() == false) {
			if (tab_intentos.getValor("intentos_recl") != null) {
				try {
					int_intentos = pckUtilidades.CConversion.CInt(tab_intentos.getValor("intentos_recl"));
				} catch (Exception e) {
				}
			}
		}
	...
	public int getIntentosAcceso(String ide_usua, String fecha) {
		...
		if (tab_intentos.isEmpty() == false) {
			try {
				int_intentos = pckUtilidades.CConversion.CInt(tab_intentos.getValor("intentos"));
			} catch (Exception e) {
			}
			...
			if (tab_desbloqueos.isEmpty() == false) {
				...
				try {
					int_num_desbloqueos = pckUtilidades.CConversion.CInt(tab_desbloqueos.getValor("desbloqueos"));
				} catch (Exception e) {
				}
	...
	public String getClaveValida(String clave) {
		...
		if (tab_regla.isEmpty() == false) {
			...
			try {
				int_longitud_minima = pckUtilidades.CConversion.CInt(tab_regla
						.getValor("longitud_minima_recl"));
			} catch (Exception e) {
			}
			try {
				int_num_carac_espe = pckUtilidades.CConversion.CInt(tab_regla
						.getValor("num_carac_espe_recl"));
			} catch (Exception e) {
			}
			try {
				int_num_mayus = pckUtilidades.CConversion.CInt(tab_regla
						.getValor("num_mayus_recl"));
			} catch (Exception e) {
			}
			try {
				int_num_minusc = pckUtilidades.CConversion.CInt(tab_regla
						.getValor("num_minusc_recl"));
			} catch (Exception e) {
			}
			try {
				int_num_numeros = pckUtilidades.CConversion.CInt(tab_regla
	...
	public boolean isClaveNueva(String ide_usua, String clave) {
		...
		int int_num_antes = 0;
		try {
			int_num_antes = pckUtilidades.CConversion.CInt(tab_reglas.getValor("NUM_VALIDA_ANTERIOR_RECL"));
		} catch (Exception e) {
		}
		if (int_num_antes == 0) {
			//valido todas
	...
	public TablaGenerica getEmpledoPartida(String ide_usua){
		return utilitario.consultar("SELECT ide_geedp,E.ide_gtemp,d.ide_geare  FROM SIS_USUARIO U" 
+ " INNER JOIN gth_empleado E ON E.IDE_GTEMP = U.IDE_GTEMP"
+" INNER JOIN gen_empleados_departamento_par D ON D.IDE_GTEMP = E.IDE_GTEMP" 
...
+" AND activo_geedp=true");
		...
		if (ide_pecl != null) {
			...
			if (tab_periodos.isEmpty() == false) {
				if (tab_periodos.getValor("num_dias") != null) {
					try {
						int_dias = pckUtilidades.CConversion.CInt(tab_periodos
								.getValor("num_dias"));
					} catch (Exception e) {
					}
				}
	...
	public int getLongitudMinimaLogin() {
		...
		if (tab_intentos.isEmpty() == false) {
			if (tab_intentos.getValor("longitud_login_recl") != null) {
				try {
					int_longitud = pckUtilidades.CConversion.CInt(tab_intentos
	...
	public String validarUsuario(String IDE_GTEMP, String nombre, String apellido, String nombreCompleto, String mail){
		
		System.out.println("validarUsuario IDE_GTEMP: "+IDE_GTEMP+", nombre: "+nombre+", apellido: "+apellido+", nombreCompleto: "+nombreCompleto+", mail: "+mail);
		
		String str_usuario="";
		String str_usuario_aux="";
		String sql="";
		String ide_usua="";
		String ide_ussu="";
		String ide_uscl="";
		...
		apellido=pckUtilidades.Utilitario.quitarCaracteresSpeciales(pckUtilidades.Utilitario.quitarAcentos(apellido).replace(".", ""));

		TablaGenerica tab_usuario = utilitario.consultar("SELECT ide_usua, ide_perf, ide_gtemp, nom_usua, nick_usua FROM sis_usuario where ide_gtemp="+IDE_GTEMP);
		TablaGenerica tab_maximo;
		
		if(tab_usuario.getTotalFilas()>0)
		{
			str_usuario=tab_usuario.getValor("nick_usua");
			return str_usuario;
		}
		else
		{
			str_usuario=nombre.toLowerCase().substring(0, 1)+apellido.toLowerCase();
			
			tab_usuario = utilitario.consultar("SELECT ide_usua, ide_gtemp, nom_usua, nick_usua FROM sis_usuario where nick_usua like '"+str_usuario+"'");
			if(tab_usuario.getTotalFilas()>0)
			{
				do
				{
					str_usuario_aux=str_usuario+sec;
					tab_usuario = utilitario.consultar("SELECT ide_usua, ide_gtemp, nom_usua, nick_usua FROM sis_usuario where nick_usua like '"+str_usuario_aux+"'");
					if(tab_usuario.getTotalFilas()>0)
					{
						sec++;
					}
				}
				while(tab_usuario.getTotalFilas()>0);
				str_usuario=str_usuario_aux;
			}
			
			tab_maximo = utilitario.consultar(ser_contabilidad.servicioCodigoMaximo("sis_usuario", "ide_usua"));
			ide_usua=tab_maximo.getValor("codigo");
			tab_maximo = utilitario.consultar(ser_contabilidad.servicioCodigoMaximo("sis_usuario_sucursal", "ide_ussu"));
			ide_ussu=tab_maximo.getValor("codigo");
			tab_maximo = utilitario.consultar(ser_contabilidad.servicioCodigoMaximo("sis_usuario_clave", "ide_uscl"));
			ide_uscl=tab_maximo.getValor("codigo");

			//////////////////////
			sql="INSERT INTO sis_usuario (ide_usua, " +
					"ide_empr, " +
					"ide_perf, " +
					"ide_gtemp, " +
					"nom_usua, " +
			...
			sql+=ide_usua+",0,2,";
			sql+=IDE_GTEMP+",'"+nombreCompleto+"','"+str_usuario+"',";
			sql+="'"+mail+"','"+utilitario.getFechaActual()+"',true,'sam',false,false);";
			
			System.out.println("sql1 "+sql);
			utilitario.getConexion().ejecutarSql(sql);
			
			//////////////////////
			sql="insert into sis_usuario_sucursal (ide_ussu, ide_sucu, sis_ide_sucu, ide_usua) values ( "+ide_ussu+",1,1,"+ide_usua+");";
			
			System.out.println("sql2 "+sql);
			utilitario.getConexion().ejecutarSql(sql);
			
			/////////////////////
			sql="insert into sis_usuario_clave (ide_uscl, ide_usua, fecha_registro_uscl, clave_uscl, activo_uscl) values ( ";
			sql+=ide_uscl+","+ide_usua+",'"+utilitario.getFechaActual()+"','"+encriptar.getEncriptar( str_usuario )+"',true);";
			
			System.out.println("sql3 "+sql);
			utilitario.getConexion().ejecutarSql(sql);
			
			System.out.println("usuario creado "+str_usuario);
