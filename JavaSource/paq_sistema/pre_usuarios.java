...
public class pre_usuarios extends Pantalla {
	...
	public pre_usuarios() {
		...
		tab_tabla1.getColumna("TEMA_USUA").setValorDefecto("home");
		...
		tab_tabla4.setTabla("sis_usuario_perfil", "ide_uspe", 4);
		tab_tabla4.getColumna("ide_perf").setCombo("sis_perfil", "ide_perf", "nom_perf", "");
		tab_tabla4.setHeader("PERFILES DE ACCESO");
		tab_tabla4.dibujar();
		...
		tab_tabla3.getColumna("ide_pecl").setCombo("sis_periodo_clave","ide_pecl", "nom_pecl", "");
		...
		tab_tabla3.getColumna("FECHA_REGISTRO_USCL").setValorDefecto(utilitario.getFechaHoraActual());
		...
		div_vertical.dividir2(pat_panel2, pat_panel4, "50%", "H");
		
		Division div_horizontal = new Division();
		div_horizontal.dividir2(pat_panel1, div_vertical, "60%", "V");
	...
	public void asignarClave(AjaxBehaviorEvent evt) {
		...
		if (tab_tabla1.isFilaInsertada()) {
			tab_tabla3.setValor("CLAVE_USCL",encriptar.getEncriptar(tab_tabla1.getValor("NICK_USUA")));
	...
	public void guardar() {
		...
		if (tab_tabla1.guardar()) {
			...
			if (tab_tabla2.guardar() && tab_tabla4.guardar()) {
				if (tab_tabla1.isFilaInsertada() == false
