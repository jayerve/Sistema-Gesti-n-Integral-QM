...
public class pre_aprobar_gastos extends Pantalla{
	...
	private static String par_sec_comprobante_pago;
	...
	public pre_aprobar_gastos() {
		...
		empleado_responsable=ser_contabilidad.empleadoResponsable(par_sec_comprobante_pago, empleado);
		
		System.out.println("empleado_responsable"+empleado_responsable);
		if(empleado_responsable==null ||empleado_responsable.isEmpty()){
			utilitario.agregarNotificacionInfo("Mensaje", "No existe usuario responsable para la aprobación de asientos de gasto/etc... ACCION NO AUTORIZADA");
			return;
		}
		...
		tab_movimientos_contables.setHeader("MOVIMIENTOS CONTABLES POR APROBAR (Gastos)");
		...
		tab_movimientos_contables.getColumna("responsable").setLongitud(50);
		tab_movimientos_contables.getColumna("aprobado").setLongitud(50);
	...
	public void seleccionaOpcion (){
		...
		String condicion=" and coalesce(ide_gemod,0) not in ("+p_modulo_anticipos+","+p_modulo_factruracion+","+p_modulo_nota_credito+","+p_modulo_nota_debito+","+p_sec_ingresos+")";
		if(com_anio.getValue()==null){
