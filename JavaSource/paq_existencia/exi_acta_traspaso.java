 ...
 * 
 */
public class exi_acta_traspaso extends Pantalla {
	...
	private void preparaDetalleCustodioEntrega() {
		...
		tab_detalle_custodio_entrega.getColumna("ide_gtemp").setCombo(ser_nomina.servicioEmpleadosActivos("true"));
		...
		tab_detalle_custodio_entrega.setCondicion("ide_afdoc=-1");
	...
	private void preparaDetalleActivo() {
		...
		tab_detalle_activo.getColumna("ide_afact").setCombo("SELECT ide_afact, ide_afact as codigo, detalle_afact,observaciones_afact,color_afact,  marca_afact, serie_afact, modelo_afact,  chasis_afact, motor_afact,cod_anterior_afact FROM afi_activo");
	...
	private void prepararSelectorActivosDelCustodio() {
		...
		set_activos_empleado.setSeleccionTabla(ser_nomina.servicioEmpleadosActivos("true"), "ide_gtemp");
	...
	public void seleccionarCustodio() {
		if (verificacionEdita()) {
			set_activos_empleado.getTab_seleccion().setSql(ser_nomina.servicioEmpleadosActivos("true"));
	...
	public void seleccionarDetalles() {
		if (tab_cabecera.getValor("ide_afdoc") != null) {
			...
			tab_detalle_custodio_entrega.setCondicion("ide_afdoc =" + tab_cabecera.getValor("ide_afdoc"));
	...
	public void guardar() {
		if (verificacionEdita()) {
			...
			tab_detalle_custodio_entrega.guardar();
			guardarPantalla();
	...
	public void actualizaActaActivos() {
		...
		for (int i = 0; i < numFil; i++) {
			tab_detalle_activo.setFilaActual(i);
			utilitario.getConexion().ejecutarSql("UPDATE  afi_activo SET afi_ultima_acta=" + acta + " WHERE ide_afact=" + pckUtilidades.CConversion.CInt(tab_detalle_activo.getValor("ide_afact")));
		}
	}
	...
	/**
	 * @param set_activos_empleado the set_activos_empleado to set
	 */
