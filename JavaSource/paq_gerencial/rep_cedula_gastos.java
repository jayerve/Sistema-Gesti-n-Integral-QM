...
public class rep_cedula_gastos extends Pantalla {
	...
	public rep_cedula_gastos() {
		...
		Object fila2[] = { "1", "CERTIFICADO" };
		Object fila3[] = { "2", "COMPROMETIDO" };
		Object fila4[] = { "3", "PAGADO" };
		lista.add(fila1);
		lista.add(fila2);
		lista.add(fila3);
		lista.add(fila4);
	...
	public void graficarPastel(List<DatoTranspuesto> datos_transpuestos,
		...
		try {
			...
			// Etiqueta
			String eti_tabla = "CEDULAS DE GASTOS";
		...
		try {
			...
			for (Columna columna : columnasIniciales) {
				...
				if (id_tipo_grafico.contentEquals("1")) {
					if (columna.getNombre().toUpperCase()
							.contentEquals("CERTIFICADO")
							|| columna.getNombre().toUpperCase()
									.contentEquals("SALDO_CERTIFICAR")) {
						columnaNueva = columna;
						columnasSeleccionadas.add(columnaNueva);
					}
				}
				...
				// Comprometido
				if (id_tipo_grafico.contentEquals("2")) {
					if (columna.getNombre().toUpperCase()
							.contentEquals("COMPROMETIDO")
							|| columna.getNombre().toUpperCase()
									.contentEquals("SALDO_COMPROMETIDO")) {
						columnaNueva = columna;
						columnasSeleccionadas.add(columnaNueva);
					}
				}
				...
				// Pagado
				if (id_tipo_grafico.contentEquals("3")) {
					if (columna.getNombre().toUpperCase()
							.contentEquals("PAGADO")
							|| columna.getNombre().toUpperCase()
									.contentEquals("SALDO_PAGADO")) {
						columnaNueva = columna;
						columnasSeleccionadas.add(columnaNueva);
					}
				}
			...
			List<DatoTranspuesto> datos_transpuestos = new ArrayList<rep_cedula_gastos.DatoTranspuesto>();
