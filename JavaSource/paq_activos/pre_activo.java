...
public class pre_activo extends Pantalla {
	...
	public pre_activo() {
		...
		tab_activos_fijos.getColumna("valor_inc_iva_afact").setEstilo("font-size:15px;font-weight: bold;text-decoration: underline;color:red");
		...
		txt_cod_inicial.setMaxlength(5);
		txt_cod_final.setMaxlength(5);
	...
	public void aceptarReporte() {
		...
		if (rep_reporte.getReporteSelecionado().equals("Ficha del Activo")) {
			if (rep_reporte.isVisible()) {
				p_parametros = new HashMap();
				rep_reporte.cerrar();
				p_parametros.put("titulo", "Ficha del Activo");
				p_parametros.put("ide_afact", Integer.parseInt(tab_activos_fijos.getValor("ide_afact")));
				self_reporte.setSeleccionFormatoReporte(p_parametros, rep_reporte.getPath());
				self_reporte.dibujar();
			}
		} else if (rep_reporte.getReporteSelecionado().equals("Activo")) {
			if (rep_reporte.isVisible()) {
				p_parametros = new HashMap();
				rep_reporte.cerrar();
			...
			if (rep_reporte.isVisible()) {
				...
				rep_reporte.cerrar();
				p_parametros.put("titulo", "Codigo de Barras");
				p_parametros.put("pide_barras", tab_activos_fijos.getValor("ide_afact"));
				self_reporte.setSeleccionFormatoReporte(p_parametros, rep_reporte.getPath());
				self_reporte.dibujar();
			}
		}
	...
	public void calcular() {
		...
		double duo_valor_inc_iva_afact = 0;
		// double duo_iva=0.12;
		...
		utilitario.addUpdateTabla(tab_activos_fijos, "valor_neto_afact,valor_compra_afact,valor_inc_iva_afact,secuencial_afact", "");
