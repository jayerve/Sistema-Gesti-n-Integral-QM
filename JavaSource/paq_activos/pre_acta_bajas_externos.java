...
/**
 * Pantalla que permite la generaci&oacute;n de actas de bajas dirigidas a
 * gestores
 * 
 * @author ccaceres
 * @since 2018/01/07
 * 
 */
public class pre_acta_bajas_externos extends Pantalla {
	...
	private SeleccionTabla set_custodio_externo = new SeleccionTabla();
	...
	// p_sec_acta_bajas_donacion
	private String parametroSecuencial = "p_sec_acta_bajas_donacion";
	private String tipoDeActa = "9";
	...
	private Dialogo dlg_desicion = new Dialogo();
	private Grid grid_desicion = new Grid();
	/**
	...
	private void preparaDetalleCustodio() {
		...
		// TODO Sacar el listado de afi_empresa_responsable
		tab_detalle_custodio.getColumna("ide_empre_res").setCombo(ser_activos.getEmpresaResponsables());
		tab_detalle_custodio.getColumna("ide_empre_res").setLectura(true);
		tab_detalle_custodio.getColumna("ide_empre_res").setAutoCompletar();
	 ...
	 * acta
	 */
	private void prepararSelectorCustodioExterno() {
		set_custodio_externo.setId("set_custodio_externo");
		set_custodio_externo.setSeleccionTabla(ser_activos.getEmpresaResponsables(), "ide_empre_res");
		set_custodio_externo.getTab_seleccion().getColumna("identificacion_empre_responsable").setFiltro(true);
		set_custodio_externo.getTab_seleccion().getColumna("detalle_empre_responsable").setFiltro(true);
		set_custodio_externo.getTab_seleccion().getColumna("cargo_empre_responsable").setFiltro(true);
		set_custodio_externo.getTab_seleccion().getColumna("detalle_empre").setFiltro(true);
		set_custodio_externo.getTab_seleccion().getColumna("identificacion_empre").setFiltro(true);
		set_custodio_externo.setTitle("Seleccione uno o varios Responsables");
		set_custodio_externo.getBot_aceptar().setMetodo("aceptarCustodiosExternos");
		agregarComponente(set_custodio_externo);
	}
	 ...
	 */
	public void seleccionarCustodioExterno() {
		if (verificacionEdita()) {
			set_custodio_externo.getTab_seleccion().setSql(ser_activos.getEmpresaResponsables());
			set_custodio_externo.getTab_seleccion().ejecutarSql();
			set_custodio_externo.dibujar();
		}
	}
	...
	/**
	 * Muestra el popup con la seleccion de si sacar activos de custodios o de
	 * bodega
	 */
	public void seleccionarDecision() {
		if (verificacionEdita()) {
			dlg_desicion.dibujar();
		}
	}
	...
	private void preparaBotones() {
		...
		bot_empleado_o.setValue("Seleccionar Activos");
		bot_empleado_o.setMetodo("seleccionarDecision");
		bar_botones.agregarBoton(bot_empleado_o);

		// boton empleado
		Boton bot_empleado = new Boton();
		bot_empleado.setIcon("ui-user");
		bot_empleado.setValue("Agregar Gestores Responsables");
		bot_empleado.setMetodo("seleccionarCustodioExterno");
	 ...
	 * Constructor
	 */
	public pre_acta_bajas_externos() {
		System.out.println("pre_acta_bajas_externos");
		...
		prepararSelectorCustodioExterno();
		...
		// Inicio Dialogo Editar Observacion
				...
				dlg_desicion.setId("dlg_desicion");
				dlg_desicion.setTitle("SELECCIONAR ORIGEN DE LOS ACTIVOS");
				dlg_desicion.setHeight("20%");
				dlg_desicion.setWidth("30%");
				grid_desicion.setColumns(2);
				...
				Boton bot_bodega_o = new Boton();
				bot_bodega_o.setIcon("ui-user");
				bot_bodega_o.setValue("Seleccionar Activos de Bodega");
				bot_bodega_o.setMetodo("seleccionarActivos");
				grid_desicion.getChildren().add(bot_empleado_o);
				grid_desicion.getChildren().add(bot_bodega_o);
				
				//grid_desicion.getChildren().add(new Etiqueta("Activos de:"));
				//grid_desicion.getChildren().add(txt_desicion);
				//dlg_desicion.getBot_aceptar()
				dlg_desicion.setDialogo(grid_desicion);
				agregarComponente(dlg_desicion);
		...
		panelDetalleCustodioE.setPanelTabla(tab_detalle_custodio);
		panelDetalleCustodioE.setTitle("En esta sección se presentan el o los custodios a los cuales se les realizó la constatación física.");
		panelDetalleCustodioE.setMensajeInfo("Custodio(s)");
		...
		// div_custodios.dividir2(panelDetalleCustodioE, panelDetalleCustodio,
		// "50%", "h");
		div_detalles.dividir2(panelDetalleActivo, panelDetalleCustodioE, "60%", "v");
	...
	public void aceptarActivoCustodio() {
		...
		System.out.print("Total filas tabla entrega: " + tab_detalle_custodio.getTotalFilas() + " empleado select " + str_seleccionado);
		if (tab_detalle_custodio.getTotalFilas() <= 0) {
			tab_detalle_custodio.insertar();
		} else {
			tab_detalle_custodio.setFilaActual(0);
		}
		tab_detalle_custodio.setValor("ide_gtemp", str_seleccionado.trim());
		tab_detalle_custodio.setValor("recibe_afddc", "FALSE");
	 ...
	 * Procesa la selecci&oacute;n en el popup de custodios
	 */
	public void aceptarCustodiosExternos() {
		String str_seleccionado = set_custodio_externo.getSeleccionados();
		// TODO terminar de sacar los custodios externos
		String[] arrray = str_seleccionado.split(",");
		System.out.println("Aceptando Externos");
		if (arrray.length > 0) {
			int numFil = tab_detalle_custodio.getTotalFilas();
			for (int i = 0; i < numFil; i++) {
				tab_detalle_custodio.setFilaActual(i);
				if (tab_detalle_custodio.getValor("recibe_afddc")=="true" && enSeleccionado(arrray, tab_detalle_custodio.getValor("ide_empre_res"))) {
					arrray = quitarDeArray(arrray, tab_detalle_custodio.getValor("ide_empre_res"));
				}
			}
		...
		if (arrray.length > 0) {
			for (String str : arrray) {
				if (!str.trim().equals("")) {
					tab_detalle_custodio.insertar();
					tab_detalle_custodio.setValor("ide_empre_res", str.trim());
					tab_detalle_custodio.setValor("recibe_afddc", "true");
				}
			}
		}
		set_custodio_externo.cerrar();
	}
	...
	public void aceptarActivos() {
		...
		// mismo custodio
		System.out.println("Aceptando Activos");
		if (arrray.length > 0) {
	...
	public void aceptarReporte() {
		if (rep_reporte.getReporteSelecionado().equals("Acta Bajas (Externos)")) {
			if (rep_reporte.isVisible()) {
				p_parametros = new HashMap();
				rep_reporte.cerrar();
				p_parametros.put("titulo", "ACTA DE BAJAS - EXTERNOS No. " + tab_cabecera.getValor("nro_secuencial_afdoc"));
	...
	public void insertar() {
		...
		if (tab_detalle_custodio.isFocus()) {
			seleccionarCustodioExterno();
		}
	}

	public void unificarCustodios() {
		// tab_detalle_custodio.insertar();
		// tab_detalle_custodio.setValor("ide_afdoc",
		// tab_cabecera.getValor("ide_afdoc"));
		// tab_detalle_custodio.setValor("ide_gtemp",
		// tab_detalle_custodio.getValor("ide_gtemp"));
		// tab_detalle_custodio.setValor("activo_afdda",
		// tab_detalle_custodio.getValor("activo_afdda"));
		// tab_detalle_custodio.setValor("recibe_afddc",
		// tab_detalle_custodio.getValor("recibe_afddc"));
		// tab_detalle_custodio.eliminar();
	}
	 ...
	 * custodios.
	 * @since 2019/05/20 Ticket #174553
	 */
	public void actualizaActaActivos() {
		int acta = pckUtilidades.CConversion.CInt(tab_cabecera.getValor("ide_afdoc"));
		String date = tab_cabecera.getValor("fecha_afdoc");
		String razon = tab_cabecera.getValor("detalle_afdoc");
		int numFil = tab_detalle_activo.getTotalFilas();
		System.out.print("Actualizando " + numFil + " activos en la baja de bienes acta " + acta + ", date " + date);
		for (int i = 0; i < numFil; i++) {
			tab_detalle_activo.setFilaActual(i);
			utilitario.getConexion().ejecutarSql(
					"UPDATE  afi_activo SET valor_en_libros="+pckUtilidades.CConversion.CDbl(tab_detalle_activo.getValor("valor_en_libros"))+", ide_afubi=" + pckUtilidades.CConversion.CInt(tab_detalle_activo.getValor("ide_afubi")) + ",afi_ubicacion_otro='" + tab_detalle_activo.getValor("afi_ubicacion_otro") + "',  fecha_baja_afact='" + date + "', razon_baja_afact='" + razon + "', afi_ultima_acta=" + acta + ", ide_boubi=8 WHERE ide_afact=" + pckUtilidades.CConversion.CInt(tab_detalle_activo.getValor("ide_afact")));
		}
	}
	...
	/**
	 * @return the set_custodio_externo
	 */
	public SeleccionTabla getSet_custodio_externo() {
		return set_custodio_externo;
	}

	/**
	 * @param set_custodio_externo
	 *            the set_custodio_externo to set
	 */
	public void setSet_custodio_externo(SeleccionTabla set_custodio_externo) {
		this.set_custodio_externo = set_custodio_externo;
	}
	...
	/**
	 * @param par_secuencial_acta
	 *            the par_secuencial_acta to set
	 */
	public static void setPar_secuencial_acta(String par_secuencial_acta) {
		pre_acta_bajas_externos.par_secuencial_acta = par_secuencial_acta;
	}
