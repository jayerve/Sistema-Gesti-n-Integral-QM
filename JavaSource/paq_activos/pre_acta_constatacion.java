 ...
 * 
 */
public class pre_acta_constatacion extends Pantalla {
	...
	private String parametroSecuencial = "p_sec_acta_constatacion";
	private String tipoDeActa = "3";
	...
	private void preparaDetalleCustodioEntrega() {
		...
		// tab_detalle_custodio.setCondicion("ide_afdoc=-1 and recibe_afddc=false");
	...
	public void seleccionarActivos(String ide_gtemp) {
		if (verificacionEdita()) {
			...
			//set_activos.getTab_seleccion().setSql(ser_activos.getActivosPorCustodio(ide_gtemp, (String) com_anio.getValue()));
	 ...
	 * Constructor
	 */
	public pre_acta_constatacion() {
		System.out.println("pre_acta_constatacion");
	...
	public void aceptarActivoCustodio() {
		...
		System.out.print("Total filas tabla entrega: " + tab_detalle_custodio.getTotalFilas());
	...
	public void aceptarReporte() {
		if (rep_reporte.getReporteSelecionado().equals("Acta de Constatacion")) {
			if (rep_reporte.isVisible()) {
				p_parametros = new HashMap();
				rep_reporte.cerrar();
				p_parametros.put("titulo", "ACTA DE CONSTATACIÓN FÍSICA No. " + tab_cabecera.getValor("nro_secuencial_afdoc"));
			...
			if (rep_reporte.isVisible()) {
				...
				p_parametros.put("titulo", "ACTA DE CONSTATACIÓN FÍSICA No. " + tab_cabecera.getValor("nro_secuencial_afdoc"));
				p_parametros.put("ide_afdoc", pckUtilidades.CConversion.CInt(tab_cabecera.getValor("ide_afdoc")));
				TablaGenerica taAni = utilitario.consultar("SELECT ide_geani,detalle_geani FROM gen_anio WHERE ide_geani=" + tab_cabecera.getValor("ide_geani"));
				p_parametros.put("anio", taAni.getValor("detalle_geani"));
				p_parametros.put("pjefe_activos", utilitario.getVariable("p_jefe_activos_fijos"));
				self_reporte.setSeleccionFormatoReporte(p_parametros, rep_reporte.getPath());
				self_reporte.dibujar();
			}
		} else if (rep_reporte.getReporteSelecionado().equals("Acta de Entrega Recepcion del acta de Constatacion")) {
			if (rep_reporte.isVisible()) {
				p_parametros = new HashMap();
				rep_reporte.cerrar();
				TablaGenerica tab_cabe = ser_activos.getActaEntregaRecepcionGenerada(tab_cabecera.getValor("ide_afdoc"));
				p_parametros.put("titulo", "ACTA ENTREGA RECEPCIÓN DE BIENES No. " + tab_cabe.getValor("nro_secuencial_afdoc"));
				p_parametros.put("ide_afdoc", pckUtilidades.CConversion.CInt(tab_cabe.getValor("ide_afdoc")));
				p_parametros.put("pjefe_activos", utilitario.getVariable("p_jefe_activos_fijos"));
				self_reporte.setSeleccionFormatoReporte(p_parametros, rep_reporte.getPath());
				self_reporte.dibujar();
			}
	...
	public void unificarCustodios() {
		...
		tab_detalle_custodio.setValor("ide_gtemp", tab_detalle_custodio.getValor("ide_gtemp"));
		tab_detalle_custodio.setValor("activo_afdda", tab_detalle_custodio.getValor("activo_afdda"));
		tab_detalle_custodio.setValor("recibe_afddc", tab_detalle_custodio.getValor("recibe_afddc"));
	...
	public void guardar() {
		if (verificacionEdita()) {
			...
			generarActaEntregaRecepcion();
		}
	}

	public void generarActaEntregaRecepcion() {
		// TODO VALIDAR si tiene ya un acta generada
		String par_secuencial_acta = utilitario.getVariable("p_sec_acta_ent_rec_cons");
		String secuencial = ser_contabilidad.numeroSecuencial(par_secuencial_acta);

		TablaGenerica tabActa = ser_activos.getActaEntregaRecepcionGenerada(tab_cabecera.getValor("ide_afdoc"));
		if (tabActa.getTotalFilas() > 0) {
			// TODO Seactualiza?
		} else {
			ser_activos.generaActaEntregaRecepcionDeConstatacion(tab_cabecera.getValor("ide_afdoc"), secuencial);
			ser_contabilidad.guardaSecuencial(secuencial, par_secuencial_acta);
			tabActa = ser_activos.getActaEntregaRecepcionGenerada(tab_cabecera.getValor("ide_afdoc"));
			ser_activos.actualizarActaERGenerada(tabActa.getValor("ide_afdoc"));
		}
	}
	...
	public void actualizaActaActivos() {
		...
		int numFil = tab_detalle_activo.getTotalFilas();
		System.out.print("Actualizando " + numFil + " activos en la constatacion de bienes acta " + acta);
		for (int i = 0; i < numFil; i++) {
			tab_detalle_activo.setFilaActual(i);
			utilitario.getConexion().ejecutarSql(
					"UPDATE  afi_activo SET ide_afubi=" + pckUtilidades.CConversion.CInt(tab_detalle_activo.getValor("ide_afubi")) + ",afi_ubicacion_otro='" + tab_detalle_activo.getValor("afi_ubicacion_otro") + "',  afi_ultima_acta_consta=" + acta
							+ " WHERE ide_afact=" + pckUtilidades.CConversion.CInt(tab_detalle_activo.getValor("ide_afact")));
		}
	}
