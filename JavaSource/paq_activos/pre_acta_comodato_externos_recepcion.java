...
/**
 * Pantalla que permite la generaci&oacute;n de actas de recepci&oacute;n de
 * comodatos emitidos por la EMGIRS EP
 * 
 * @author ccaceres
 * @since 2019/05/17
 * 
 */
public class pre_acta_comodato_externos_recepcion extends Pantalla {
	...
	private String parametroSecuencial = "p_sec_acta_comodato_externo_recepcion";
	private String tipoDeActa = "17";
	...
	private void prepararSelectorActivo() {
		...
		set_activos.setSeleccionTabla(ser_activos.getActivosPorTipoUltimaActa("16,10"), "ide_afact");
	...
	public void seleccionarActivos() {
		if (verificacionEdita()) {
			set_activos.getTab_seleccion().setSql(ser_activos.getActivosPorTipoUltimaActa("16,10"));
	...
	private void preparaBotones() {
		...
		bot_empleado_o.setValue("Seleccionar Activos en Comodato");
		bot_empleado_o.setMetodo("seleccionarActivos");
		bar_botones.agregarBoton(bot_empleado_o);

		// boton empleado
		Boton bot_empleado = new Boton();
		bot_empleado.setIcon("ui-user");
		bot_empleado.setValue("Agregar Gestores que Entregan");
		bot_empleado.setMetodo("seleccionarCustodioExterno");
		bar_botones.agregarBoton(bot_empleado);
		// boton empleado
		Boton bot_empleado_in = new Boton();
		bot_empleado_in.setIcon("ui-user");
		bot_empleado_in.setValue("Agregar Custodios que Reciben");
		bot_empleado_in.setMetodo("seleccionarCustodios");
		bar_botones.agregarBoton(bot_empleado_in);
	 ...
	 * Constructor
	 */
	public pre_acta_comodato_externos_recepcion() {
		System.out.println("pre_acta_comodato_externos_recepcion");
	...
	public void aceptarCustodiosExternos() {
		...
		if (arrray.length > 0) {
			...
			for (int i = 0; i < numFil; i++) {
				tab_detalle_custodio.setFilaActual(i);
				if (tab_detalle_custodio.getValor("recibe_afddc") == "false" && enSeleccionado(arrray, tab_detalle_custodio.getValor("ide_empre_res"))) {
	...
	public void aceptarReporte() {
		if (rep_reporte.getReporteSelecionado().equals("Acta Recepcion Comodato")) {
			if (rep_reporte.isVisible()) {
				p_parametros = new HashMap();
				rep_reporte.cerrar();
				p_parametros.put("titulo", "ACTA DE RECEPCIÓN DE COMODATO No. " + tab_cabecera.getValor("nro_secuencial_afdoc"));
	...
	public void actualizaActaActivos() {
		...
		for (int i = 0; i < numFil; i++) {
			...
			utilitario.getConexion().ejecutarSql(
					"UPDATE  afi_activo SET valor_en_libros="+pckUtilidades.CConversion.CDbl(tab_detalle_activo.getValor("valor_en_libros"))+", ide_afubi=" + pckUtilidades.CConversion.CInt(tab_detalle_activo.getValor("ide_afubi")) + ",afi_ubicacion_otro='" + tab_detalle_activo.getValor("afi_ubicacion_otro") + "',  fecha_baja_afact='" + date
							+ "', razon_baja_afact='" + razon + "', afi_ultima_acta=" + acta + ", ide_boubi=8 WHERE ide_afact=" + pckUtilidades.CConversion.CInt(tab_detalle_activo.getValor("ide_afact")));
		}
	}
	...
	public static void setPar_secuencial_acta(String par_secuencial_acta) {
		pre_acta_comodato_externos_recepcion.par_secuencial_acta = par_secuencial_acta;
	}
