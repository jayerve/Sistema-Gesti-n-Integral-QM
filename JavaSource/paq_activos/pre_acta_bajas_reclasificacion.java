...
/**
 * Pantalla que permite la generaci&oacute;n de actas de bajas
 * 
 * @author ccaceres
 * @since 2017/08/07
 * 
 */
public class pre_acta_bajas_reclasificacion extends Pantalla {
	...
	private String parametroSecuencial = "p_sec_acta_bajas_reclasificacion";
	private String tipoDeActa = "18";
	...
	private void preparaCabecera() {
		...
		tab_cabecera.getColumna("ide_afdoc").setNombreVisual("CÓDIGO");
		tab_cabecera.getColumna("ide_afdoc").setVisible(true);
		tab_cabecera.getColumna("ide_afdoc").setLectura(false);
		
		tab_cabecera.getColumna("ide_aftidoc").setNombreVisual("TIPO");
		tab_cabecera.getColumna("ide_aftidoc").setVisible(true);
		tab_cabecera.getColumna("ide_aftidoc").setLectura(true);
		
		tab_cabecera.getColumna("ide_geani").setNombreVisual("TIPO");
		tab_cabecera.getColumna("ide_geani").setVisible(false);
		tab_cabecera.getColumna("ide_geani").setLectura(false);
		
		tab_cabecera.getColumna("ide_gtemp").setNombreVisual("RESPONSABLE");
		tab_cabecera.getColumna("ide_gtemp").setVisible(true);
		tab_cabecera.getColumna("ide_gtemp").setLectura(true);
		
		tab_cabecera.getColumna("ide_gtemp_aut").setNombreVisual("AUTORIZADOR");
		tab_cabecera.getColumna("ide_gtemp_aut").setVisible(false);
		tab_cabecera.getColumna("ide_gtemp_aut").setLectura(false);
		
		tab_cabecera.getColumna("nro_secuencial_afdoc").setNombreVisual("ACTA");
		tab_cabecera.getColumna("nro_secuencial_afdoc").setVisible(true);
		tab_cabecera.getColumna("nro_secuencial_afdoc").setLectura(false);
		
		tab_cabecera.getColumna("detalle_afdoc").setNombreVisual("OBSERVACIONES");
		tab_cabecera.getColumna("detalle_afdoc").setVisible(true);
		tab_cabecera.getColumna("detalle_afdoc").setLectura(false);
		
		tab_cabecera.getColumna("fecha_afdoc").setNombreVisual("FECHA");
		tab_cabecera.getColumna("fecha_afdoc").setVisible(true);
		tab_cabecera.getColumna("fecha_afdoc").setLectura(false);
		
		tab_cabecera.getColumna("activo_afdoc").setNombreVisual("ACTIVO");
		
		tab_cabecera.getColumna("ide_boubi").setVisible(false);
		tab_cabecera.getColumna("aprobacion_adm").setVisible(false);
		tab_cabecera.getColumna("ide_gtemp_jef_aut").setVisible(false);
	...
	private void preparaDetalleCustodioEntrega() {
		...
		// tab_detalle_custodio.setTabla("afi_doc_detalle_custodio",
		// "ide_afddc", 4);
		//
	...
	private void preparaBotones() {
		...
		bot_empleado.setValue("Agregar Custodios Compartidos");
	 ...
	 * Constructor
	 */
	public pre_acta_bajas_reclasificacion() {
		System.out.println("pre_acta_bajas_reclasificacion");
	...
	public void aceptarReporte() {
		if (rep_reporte.getReporteSelecionado().equals("Acta Bajas Por Reclasificación")) {
			if (rep_reporte.isVisible()) {
				p_parametros = new HashMap();
				rep_reporte.cerrar();
				p_parametros.put("titulo", "ACTA DE BAJAS POR RECLASIFICACIÓN No. " + tab_cabecera.getValor("nro_secuencial_afdoc"));
	...
	public void actualizaActaActivos() {
		...
		for (int i = 0; i < numFil; i++) {
			...
			utilitario.getConexion().ejecutarSql(
					"UPDATE  afi_activo SET valor_en_libros=" + pckUtilidades.CConversion.CDbl(tab_detalle_activo.getValor("valor_en_libros")) + ", ide_afubi=" + pckUtilidades.CConversion.CInt(tab_detalle_activo.getValor("ide_afubi"))
							+ ",afi_ubicacion_otro='" + tab_detalle_activo.getValor("afi_ubicacion_otro") + "',  fecha_baja_afact='" + date + "', razon_baja_afact='" + razon + "', afi_ultima_acta=" + acta + ", ide_boubi=8 WHERE ide_afact="
							+ pckUtilidades.CConversion.CInt(tab_detalle_activo.getValor("ide_afact")));
		}
	}
	...
	/**
	 * @return the dlg_desicion
	 */
	public Dialogo getDlg_desicion() {
		return dlg_desicion;
	}

	/**
	 * @param dlg_desicion the dlg_desicion to set
	 */
	public void setDlg_desicion(Dialogo dlg_desicion) {
		this.dlg_desicion = dlg_desicion;
	}
	/**
	 * @return the grid_desicion
	 */
	public Grid getGrid_desicion() {
		return grid_desicion;
	}

	/**
	 * @param grid_desicion the grid_desicion to set
	 */
	public void setGrid_desicion(Grid grid_desicion) {
		this.grid_desicion = grid_desicion;
	}
}
